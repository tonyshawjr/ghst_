...
require_once '../../../includes/platforms/InstagramPlatform.php';
...
if ($error) {
    $_SESSION['error'] = 'Instagram OAuth error: ' . ($_GET['error_description'] ?? $error);
...
try {
    ...
    $redirectUri = APP_URL . '/api/oauth/callback/instagram.php';
    $tokenData = $instagram->handleCallback($code, $state, $redirectUri);
    
    // Check if account already exists
    $stmt = $db->prepare("
        SELECT id FROM accounts 
        WHERE client_id = ? AND platform = 'instagram' AND platform_user_id = ?
    ");
    ...
    if ($existingAccount) {
        ...
        $_SESSION['success'] = 'Instagram account reconnected successfully!';
    } else {
        // Create new account
        $stmt = $db->prepare("
            ...
            ) VALUES (?, 'instagram', ?, ?, ?, NULL, ?, 1, NOW(), NOW())
        ");
        $stmt->execute([
            $client['id'],
        ...
        $_SESSION['success'] = 'Instagram account connected successfully!';
    }
    ...
    $_SESSION['error'] = 'Failed to connect Instagram account: ' . $e->getMessage();
}
