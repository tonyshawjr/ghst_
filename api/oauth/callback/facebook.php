...
require_once '../../../includes/platforms/FacebookPlatform.php';
...
if ($error) {
    $_SESSION['error'] = 'Facebook OAuth error: ' . ($_GET['error_description'] ?? $error);
...
try {
    ...
    // Create Facebook platform instance
    $facebook = new FacebookPlatform();
    
    // Exchange code for access token
    $redirectUri = APP_URL . '/api/oauth/callback/facebook.php';
    $tokenData = $facebook->handleCallback($code, $state, $redirectUri);
    
    // Check if account already exists
    $stmt = $db->prepare("
        SELECT id FROM accounts 
        WHERE client_id = ? AND platform = 'facebook' AND platform_user_id = ?
    ");
    ...
    if ($existingAccount) {
        ...
        $_SESSION['success'] = 'Facebook account reconnected successfully!';
    } else {
        // Create new account
        $stmt = $db->prepare("
            ...
            ) VALUES (?, 'facebook', ?, ?, ?, NULL, ?, 1, NOW(), NOW())
        ");
        $stmt->execute([
            $client['id'],
        ...
        $_SESSION['success'] = 'Facebook account connected successfully!';
    }
    ...
    $_SESSION['error'] = 'Failed to connect Facebook account: ' . $e->getMessage();
}
